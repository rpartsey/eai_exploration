...
from habitat_extensions.habitat_lab.tasks.exp.nav import StepCountSensor
...
class PointNavResNetNet(Net):
    ...
    def __init__(
        ...
        if StepCountSensor.cls_uuid in observation_space.spaces:
            input_step_count = (
                observation_space.spaces[StepCountSensor.cls_uuid].shape[0]
            )
            self.step_count_embedding = nn.Linear(input_step_count, 32)
            rnn_input_size += 32
    ...
    def forward(
        ...
        if StepCountSensor.cls_uuid in observations:
            sensor_observations = observations[StepCountSensor.cls_uuid]
            x.append(self.step_count_embedding(sensor_observations))
